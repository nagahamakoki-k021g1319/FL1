...
{
	sparkParticle_ = make_unique<ParticleManager>();
	sparkParticle_->Initialize();
	sparkParticle_->LoadTexture("particle.png");

	//指定した名前のグループを作る
	GlobalVariables* globalVariables = GlobalVariables::GetInstance();
	globalVariables->CreateGroup(groupName_);
	//第一引数名のグループ内に第二引数名の調整項目が無ければ、第三引数を初期値として登録
	globalVariables->AddItem(groupName_, "randPos", 0.4f);
	globalVariables->AddItem(groupName_, "randVel", 0.02f);
	globalVariables->AddItem(groupName_, "randVelY", 0.0f);
	globalVariables->AddItem(groupName_, "startScale", 0.3f);
...
void Spark::ApplyGlobalVariables()
{
...
void Spark::EffSummary(Vector3 pos)
{
	//パーティクル範囲
	for (int i = 0; i < 20; i++) {
		...
		velGas.z = (float)rand() / RAND_MAX * randVel - randVel / 2.0f;
		//重力に見立ててYのみ[-0.001f,0]でランダムに分布
		Vector3 accGas{};
		accGas.x = (float)rand() / RAND_MAX * rnd_accGas - rnd_accGas / 2.0f;
		accGas.y = (float)rand() / RAND_MAX * rnd_accGas - rnd_accGas / 2.0f;
		//追加
		sparkParticle_->Add(liveTime, posGas, velGas, accGas, startScale, endScale, { color.x,color.y,color.z,alpha });
	}
}
